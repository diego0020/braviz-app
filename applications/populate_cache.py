...
def clear_pickles():
    #pickles
    reader=braviz.readAndFilter.kmc40AutoReader(max_cache=5) #small cache
...
def clear_all():
    reader=braviz.readAndFilter.kmc40AutoReader(max_cache=1)
    ids=reader.get('ids')
    for i in ids:
        clear_cache(i)
    clear_pickles()
...
def populate_cache(subj):
    ...
    reader2=ignoring_reader(max_cache=5) #small cache
    #Dartel Transform
    print " %s: Dartel"%subj
