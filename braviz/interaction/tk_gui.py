...
class subjects_list(LabelFrame):
    ...
    def __init__(self, reader, function, master, **kw):
        ...
        subjects_list_box = tk.Listbox(
            list_and_bar, selectmode=tk.BROWSE, yscrollcommand=scrollbar.set, exportselection=0, listvariable=tk_listvariable)
...
class structureList(LabelFrame):
    ...
    def __init__(self, reader, initial_subj, command, parent, initial_models=None, **kw):
        ...
        model_list = tk.Listbox(model_list_and_bar, selectmode=tk.MULTIPLE,
                                yscrollcommand=model_scrollbar.set, exportselection=0, listvariable=self.tk_listvariable)
        ...
        self.tool_tip = ToolTip(
            model_list, msgFunc=self.__get_tooltip, follow=1, delay=0.5)
    ...
    def __update(self, *args):
        ...
        if new_set:
            if len(new_set) > 1:
                log = logging.getLogger(__name__)
                log.warning(
                    "WARNING: this shouldn't happen, model_idx changed by more than one")
        ...
        if len(childs) > 0:
            if tooltip_source is None:
                hierarchy_dict_to_tree(
                    tree_view, childs, iid, tags, tooltip_dict, tooltip_source, '')
            else:
                hierarchy_dict_to_tree(
                    tree_view, childs, iid, tags, tooltip_dict, tooltip_source, tooltip_source.get(name, ''))
