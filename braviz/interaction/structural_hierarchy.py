...
def get_structural_hierarchy(reader, subj, dom_nondom=True, right_left=True, fibers=True):
    ...
    for struct in structure_names:
        if struct.endswith('-SPHARM'):
            ...
            hemisphere = 'Right Hemisphere' if struct[
                4] == 'r' else 'Left Hemisphere'
            name = struct[7:]
            ...
            if dom_nondom is True:
                ...
                hierarchy_dict['Nondominant Hemisphere'][
                    'All Gray Matter'][name]
...
def get_structural_hierarchy_with_names(reader, subj, dom_nondom=True, right_left=True, fibers=True):
    ...
    for struct in structure_names:
        if struct.endswith('-SPHARM'):
            ...
            if dom_nondom is True:
                hierarchy_dict['Dominant Hemisphere'][
                    'All Gray Matter'][name] = "ctx-dh-" + name
                hierarchy_dict['Nondominant Hemisphere'][
                    'All Gray Matter'][name] = "ctx-nh-" + name
                hierarchy_dict['Dominant Hemisphere'][
                    name]['Gray Matter'] = "ctx-dh-" + name
                hierarchy_dict['Nondominant Hemisphere'][
                    name]['Gray Matter'] = "ctx-nh-" + name
        elif struct.startswith('wm-'):
            hemisphere = 'Right Hemisphere' if struct[
                3] == 'r' else 'Left Hemisphere'
            name = struct[6:]
            ...
            if dom_nondom is True:
                hierarchy_dict['Dominant Hemisphere'][
                    'All White Matter'][name] = "wm-dh-" + name
                hierarchy_dict['Nondominant Hemisphere'][
                    'All White Matter'][name] = "wm-nh-" + name
                hierarchy_dict['Dominant Hemisphere'][
                    name]['White Matter'] = "wm-dh-" + name
                hierarchy_dict['Nondominant Hemisphere'][
                    name]['White Matter'] = "wm-nh-" + name
        elif struct.startswith('CC'):
            name = struct[3:]
