...
class BarPlot(object):
    ...
    def __init__(self, tight=True):
        ...
        f = Figure(dpi=None, tight_layout=tight,
                   facecolor='w', frameon=False, edgecolor='r')
    ...
    def get_widget(self, master, **kwargs):
        ...
        widget.configure(
            bd=4, bg='white', highlightcolor='red', highlightthickness=0, **kwargs)
        ...
        cid = self.figure.canvas.mpl_connect(
            'motion_notify_event', update_mouse_pos)
        ...
        cid2 = self.figure.canvas.mpl_connect(
            'button_press_event', click_on_bar)
        widget.focus()
        self.widget = widget
        return widget
    ...
    def paint_bar_chart(self):
        ...
        if self.style == 'bars':
            if self.back_bars is not None:
                ...
                back_colors = map(
                    self.color_function, back_heights, back_names)
                ...
                offsets = [0] * back_widths[0] + [1] * \
                    back_widths[1] + [2] * back_widths[2]
            ...
            patches = a.bar(
                bar_positions, self.bar_heights, color=colors, align='center', yerr=self.yerror)
            ...
            patches = a.scatter(bar_positions, self.bar_heights, c=colors,
                                marker='s', s=sizes, edgecolors=edge_colors, linewidths=linewidths)
            if self.yerror is not 0:
                a.errorbar(
                    bar_positions, self.bar_heights, yerr=self.yerror, fmt=None)
    ...
    def change_bars(self, new_heights):
        ...
        if self.style == 'bars':
            patches = a.bar(
                bar_positions, self.bar_heights, color=colors, align='center')
        else:
            patches = a.scatter(
                bar_positions, self.bar_heights, c=colors, marker='s', s=40)
        self.bars = patches
        self.show()
...
class ScatterPlot(object):
    ...
    def __init__(self, tight=True, use_ransac=True):
        ...
        f = Figure(
            tight_layout=tight, facecolor='w', frameon=False, edgecolor='r')
        a = f.gca(axisbg='w')
        self.figure = f
        self.axis = a
        self.widget = None
    ...
    def get_widget(self, master, **kwargs):
        ...
        cid2 = self.figure.canvas.mpl_connect(
            'button_press_event', generate_tk_event)
        self.widget = widget
        ...
        cid2 = self.figure.canvas.mpl_connect(
            'button_press_event', click_on_scatter)
        if self.tight is True:
            self.resizing = None
    ...
    def draw_scatter(self):
        ...
        if self.__highlited is not None:
            with ignored(ValueError):
                ...
                sizes[idx] = 40
                linewidths[idx] = 2
        self.__paths = a.scatter(self.x_values, self.y_values, color=colors,
                                 picker=1, s=sizes, linewidths=linewidths, edgecolor=highlight_color)

        reg_line, r_value = self.get_regression_line(
            None, self.x_values, self.y_values)

        a.plot(self.x_limits, map(reg_line, self.x_limits),
               c='k', label="all (r=%.2f)" % r_value)
        if self.__groups_dict is not None:
            ...
            for g in groups:
                g_idxs = filter(lambda i: self.__groups_dict[
                                self.__data_names[i]] == g, xrange(len(self.__data_names)))
                # reg_line,r=calculate_regression_line(x_data[g_idxs],y_data[g_idxs])
                reg_line, r = self.get_regression_line(
                    g, x_data[g_idxs], y_data[g_idxs])
                a.plot(self.x_limits, map(reg_line, self.x_limits),
                       c=self.__color_fun(0, g), label="%s (r=%.2f)" % (g, r))
...
def calculate_ransac_regression(x, y):
    ...
    try:
        ransac_fit, ransac_data = ransac.ransac(all_data, model,
                                                # misc. parameters
                                                n_fit, iterations, error_threshold, additiona_fit,
...
class SpiderPlot(object):
    ...
    def get_widget(self, master, **kwargs):
        ...
        cid2 = self.figure.canvas.mpl_connect(
            'button_press_event', click_on_spider)
        self.widget = widget
        widget.focus()
        return widget
    ...
    def draw_spider(self):
        ...
        for i, g in enumerate(group_list):
            ...
            custom_pick = self.get_custom_picker_function(
                filtered_keys, pickers_dict)
...
#            print theta
            ...
            if index == len(patches):
                index -= 1
            # print "%f in [%f , %f)
            # "%(theta,patches[index][0],patches[index+1][0])
            t1, r1 = patches[index]
